...
scaler_y_stock_test.fit(y_train_np.reshape(-1, 1))  # Passe den Scaler auf die Trainingsdaten an
...
predictions_rescaled = scaler_y_stock_test.inverse_transform(predictions_np)

y_test_np = y_test.numpy()  # Konvertiere y_test in NumPy
y_test_rescaled = scaler_y_stock_test.inverse_transform(y_test_np.reshape(-1, 1))
...
plt.legend()
plt.title('Vorhersagen vs. Echte Werte von AAPL mit HLOCV-Daten')
plt.show()
...
plt.title('Learning Curve (Loss) von AAPL mit HLOCV-Daten')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
...
if 'mae' in history.history:
    ...
    plt.title('Learning Curve (MAE) von AAPL mit HLOCV-Daten')
    plt.xlabel('Epoch')
