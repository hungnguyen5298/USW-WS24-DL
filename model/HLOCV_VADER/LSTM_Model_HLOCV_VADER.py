...
X_train = np.load('vader_train_X.npy')
y_train = np.load('vader_train_y.npy')
X_train = tf.convert_to_tensor(X_train, dtype=tf.float32)
y_train = tf.convert_to_tensor(y_train, dtype=tf.float32)

X_val = np.load('vader_val_X.npy')
y_val = np.load('vader_val_y.npy')
X_val = tf.convert_to_tensor(X_val, dtype=tf.float32)
y_val = tf.convert_to_tensor(y_val, dtype=tf.float32)

X_test = np.load('vader_test_X.npy')
y_test = np.load('vader_test_y.npy')
...
# Modell speichern
model.save('lstm_model_hlocv_vader.keras')
...
scaler_y_vader_test.fit(y_train_np.reshape(-1, 1))  # Passe den Scaler auf die Trainingsdaten an
...
predictions_rescaled = scaler_y_vader_test.inverse_transform(predictions_np)

y_test_np = y_test.numpy()  # Konvertiere y_test in NumPy
y_test_rescaled = scaler_y_vader_test.inverse_transform(y_test_np.reshape(-1, 1))
...
plt.legend()
plt.title('Vorhersagen vs. Echte Werte von AAPL mit HLOCV-Daten + Sentimentdaten aus VADER')
plt.show()
...
plt.title('Learning Curve (Loss) von AAPL mit HLOCV-Daten + Sentimentdaten aus VADER')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
...
if 'mae' in history.history:
    ...
    plt.title('Learning Curve (MAE) von AAPL mit HLOCV-Daten + Sentimentdaten aus VADER')
    plt.xlabel('Epoch')
